...
async def _get_client(device_id: int, db: AsyncSession):
...
async def get_interfaces(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_interfaces()


@router.get("/{device_id}/addresses")
async def get_addresses(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_ip_addresses()


@router.get("/{device_id}/routes")
async def get_routes(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_routes()


@router.get("/{device_id}/neighbors")
async def get_neighbors(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_neighbors()


@router.get("/{device_id}/firewall")
async def get_firewall(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_firewall_rules()


@router.get("/{device_id}/wireless")
async def get_wireless(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_wireless()


@router.get("/{device_id}/dhcp-leases")
async def get_dhcp_leases(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_dhcp_leases()


@router.get("/{device_id}/tunnels")
async def get_tunnels(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
    return await client.get_vpn_tunnels()


@router.get("/{device_id}/resource")
async def get_resource(device_id: int, db: AsyncSession = Depends(get_db)):
    client, _ = await _get_client(device_id, db)
