...
try {
    ...
    switch ($action) {
        ...
        // ── Mikrotik DRP documents — same sensitivity/gating as the backup
        // actions just above (full device configuration, encrypted, but
        // still admin-only to list/download). Uploaded by ovh/drp.php,
        // generated by services/drp_docs.py. ─────────────────────────────
        ...
        case 'drp_doc_list':
            ...
            $stmt = $pdo->prepare(
                'SELECT id, created_at, size_bytes FROM drp_documents WHERE tenant = ? ORDER BY created_at DESC LIMIT 20'
            );
            $stmt->execute([$tenant]);
            echo json_encode(['documents' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
            break;

        case 'drp_doc_download':
            $tenant = $_GET['tenant'] ?? '';
            ...
            $stmt = $pdo->prepare(
                'SELECT payload, created_at, size_bytes FROM drp_documents WHERE id = ? AND tenant = ?'
            );
            $stmt->execute([$doc_id, $tenant]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$row) {
                http_response_code(404);
                echo json_encode(['error' => 'not found']);
            ...
            if (!is_array($envelope)) {
                http_response_code(500);
                echo json_encode(['error' => 'stored document is not valid JSON']);
                break;
            }
            echo json_encode([
                'created_at' => $row['created_at'],
        ...
        case 'request_drp_doc_generate':
            // Queue a DRP-document generation pass for a specific tenant —
            // mirrors request_linux_scan/request_supply_chain_scan exactly
            // (bare tenant-scoped marker, no host id needed).
            $tenant = $_GET['tenant'] ?? '';
            ...
            file_put_contents($state_dir . '/drp_doc_generate_pending_' . $safe, date('c'));
            echo json_encode(['ok'=>true,'tenant'=>$tenant,'queued_at'=>date('c'),'note'=>'Delivered on next heartbeat (max 2 min); document upload follows once generation finishes']);
            break;

        case 'pending_drp_doc_generates':
            require_global($identity);
            $state_dir = $config['state_dir'] ?? __DIR__ . '/state';
            $pending = [];
            if (is_dir($state_dir)) {
                foreach (glob($state_dir . '/drp_doc_generate_pending_*') as $f) {
                    $tenant = substr(basename($f), strlen('drp_doc_generate_pending_'));
        ...
        case 'tunnel_status_all':
            ...
            // snapshot payload per tenant, no E2E key needed since
            // tunnel_status travels as plaintext envelope metadata (see
            // services/tunnel_monitor.py's public_summary()).
            $stmt = $pdo->query(
                'SELECT t.id AS tenant, t.last_seen,
                        (SELECT payload FROM snapshots
                         WHERE tenant = t.id
            ...
            foreach ($rows as $r) {
                ...
                if (!empty($r['_latest_payload'])) {
                    $meta = json_decode($r['_latest_payload'], true);
                    if (is_array($meta)) { $tunnels = $meta['tunnel_status'] ?? []; }
                }
                $result[] = [
                    'tenant' => $r['tenant'],
                    'last_seen' => $r['last_seen'],
        ...
        case 'compliance_status_all':
            // Aggregate view across every tenant this identity can see —
            // same subquery pattern as wan_links_status_all/tunnel_status_all:
            // latest snapshot payload per tenant, no E2E key needed since
            // compliance_status travels as plaintext envelope metadata (see
            // services/compliance.py's public_summary() — FAILED checks
            // only, i.e. actual recommendations to act on).
            $stmt = $pdo->query(
                'SELECT t.id AS tenant, t.last_seen,
                        (SELECT payload FROM snapshots
                         WHERE tenant = t.id
            ...
            foreach ($rows as $r) {
                ...
                if (!empty($r['_latest_payload'])) {
                    $meta = json_decode($r['_latest_payload'], true);
                    if (is_array($meta)) { $findings = $meta['compliance_status'] ?? []; }
                }
                $result[] = [
                    'tenant' => $r['tenant'],
                    'last_seen' => $r['last_seen'],
