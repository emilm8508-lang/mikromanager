...
import { Server, Wifi, CheckCircle2, XCircle, Clock } from 'lucide-react'
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Przegląd sieci Mikrotik</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Wszystkich urządzeń" value={devices.length} icon={Server} color="bg-indigo-600/20 text-indigo-400" />
        <StatCard label="Online" value={online} icon={CheckCircle2} color="bg-green-600/20 text-green-400" />
        <StatCard label="Offline" value={offline} icon={XCircle} color="bg-red-600/20 text-red-400" />
        <StatCard label="Bez poświadczeń" value={devices.filter(d => !d.credential_id).length} icon={Wifi} color="bg-yellow-600/20 text-yellow-400" />
      </div>

      <Card>
        <CardHeader>
          <h2 className="text-sm font-semibold text-gray-300">Ostatnio widziane urządzenia</h2>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <p className="px-5 py-8 text-center text-gray-500 text-sm">Ładowanie...</p>
          ) : devices.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <Server size={32} className="mx-auto text-gray-700 mb-3" />
              <p className="text-gray-500 text-sm">Brak urządzeń. Uruchom skan sieci.</p>
              <Link to="/scanner" className="text-indigo-400 text-sm hover:underline mt-1 inline-block">Przejdź do skanera →</Link>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800 text-xs text-gray-500">
                  ...
                  <th className="px-5 py-3 text-left">Tożsamość</th>
