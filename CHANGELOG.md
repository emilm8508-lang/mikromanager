...
## 2.32 — 2026-09-23
- Add a ManageEngine ServiceDesk Plus (on-premise) connector, configured on the one tenant that already has SDP installed - a first step toward using it as the NIS2/KSC incident-and-request register instead of building a separate one from scratch. Mirrors the PRTG/Check_MK/Wazuh connectors exactly: connection settings (URL + technician Authtoken, encrypted at rest, same key-rotation lifecycle) plus a read-only "test connection" call, in Central's Agent tab. Deliberately settings-only for now - a ready-to-use create_ticket() (creates either an "Incident" or a "Service Request" via SDP's REST API v3, confirmed against ManageEngine's own API reference) exists but isn't wired to any alert type yet, since which events should become tickets is a separate decision for later.

## 2.31 — 2026-09-22
- Fix false "WAN łącze nie działa" alerts caused by OVH's own network being the unreachable one, not the client's WAN (surfaced live during a recent OVH-side connectivity incident, where 3 tenants' agents lost contact with Central and their edge-device monitoring started reporting every WAN link down, even though the links themselves were fine). Before Central now actually fires an offline alert for an edge device, it asks 1-2 OTHER tenants' agents (picked at random, only among ones with a recent heartbeat) to independently ping/TCP-check the SAME address first - an outside vantage point that isn't affected by whatever made the original check fail (OVH's own flaky ICMP-less probe, or the reporting agent itself being unreachable). The alert only actually fires once that independent check also fails, or after a 15-minute timeout if nobody was available to ask (never silently suppresses a real outage). Deliberately does NOT reintroduce direct agent-to-agent communication (ruled out earlier): the target address travels OVH -> agent through the same signed command channel as every other Central-to-agent command, tagged only with an opaque verification id - the checking agent never learns which tenant actually owns the address it was asked to ping, and its result goes back to OVH only.

## 2.30 — 2026-09-20
- Fix Windows Update installation and remote restart both failing on every managed host (surfaced immediately by the new "Recent actions" panel: `schtasks /Create failed: ERROR: Invalid argument/option - '-NoProfile'` across every tenant). Root cause, confirmed against real Windows argv parsing (CommandLineToArgvW), not guessed: pywinrm's `run_cmd(exe, args)` does NOT protect a multi-word argument from being split once WinRS rejoins it server-side - so schtasks.exe's own command-line parser saw `/TR`'s value as just "powershell.exe", then choked on "-NoProfile" as an unrecognized top-level argument of its own, and the same thing was silently waiting for `shutdown`'s `/c <reason>` the moment anyone used a multi-word restart reason. Fixed by wrapping both values in their own quotes (backslash-escaping the inner `-File "path"` quotes for the /TR case - the standard Windows idiom for a quoted value inside a quoted value), verified against the real OS parser rather than assumed. Also switched the install script's temp-file paths from `$env:TEMP` to a fixed `C:\Windows\Temp`: two more bugs that were hiding behind the first one - (1) the install script runs as SYSTEM while the code reading its result file back runs as the WinRM-authenticated user, and `$env:TEMP` resolves to a different physical folder for each, so the result would never actually be found even once schtasks itself worked; (2) `$env:TEMP` baked into the schtasks `/TR` value is inert literal text once Task Scheduler launches `powershell.exe` directly (confirmed: `-File "$env:TEMP\x"` fails outright with "Specify a valid path for the -File parameter") - it only ever expanded correctly in the *other* two places this code already used it (via genuine remote PowerShell execution), never in this one. This was the least-verified part of the whole Windows management module (per its own docstring) and had never been exercised against a real host before now.

## 2.29 — 2026-09-20
- Add a "Recent actions" panel to Central's main (Widok) tab — a single place to see everything requested across the many separate trigger-an-action buttons this app has accumulated (agent update/restart, firmware upgrade, Linux/Windows apt-upgrade/restart, DRP doc generation, Dell checks, vuln remediation, router classification, log fetch, Windows-manage toggle), with a status per entry: queued, delivered to the agent, done, or failed. Backed by a new ovh/api.php `recent_actions` action that merges two things that already existed separately but were never shown together: still-pending marker files (queued) and the matching activity_log rows ingest.php already writes when a marker is drained ("*_delivered") plus whatever completion an action reports back on its own (activity.record() events, already flowing through the existing activity_events pipeline). Along the way, closed two gaps that would have left entries permanently stuck at "queued": firmware-upgrade and device-log-fetch commands never logged their own "delivered" event (now fixed), and DRP document generation never reported success/failure back to Central at all (added) - plus a handful of new activity_format_message() cases so Linux/Windows apt-upgrade/restart entries read as an actual sentence instead of the generic "type: ?" fallback.

## 2.28 — 2026-09-19
- Add Mikrotik disaster-recovery (DRP) documentation, generated on demand from Central. For every Mikrotik device not marked "standard configuration" (new per-device toggle in Devices → edit, for e.g. a switch where only the admin password was ever changed), pulls the full plain-text `/export` of its configuration over SSH (mikrotik_client.py's new get_config_export() — the one thing REST/API/SNMP can't give directly, since it's a CLI console command, not a queryable resource) plus model/RouterOS version/credential NAME (never the password) and LLDP/CDP neighbors, and assembles it into one Word document. Deliberately Mikrotik-only: RouterOS's /export is a genuinely restorable script (pasteable straight into a replacement device's console), which no general-purpose OS has an equivalent of - Linux/Windows hosts were explicitly out of scope for this. Click "DRP" next to a tenant in Central to queue generation and download the result once ready; the document is also always saved locally under data/drp_exports/ on the agent itself, and - when Central/E2E encryption is configured - uploaded there the same encrypted way agent-state backups already are (new ovh/drp.php + drp_documents table, mirroring ovh/backup.php almost verbatim), so a consultant managing several client sites doesn't need to open each one's own agent UI just to pull this.
...
## 2.25 — 2026-09-15
- Add a Wazuh connector, alongside the existing PRTG/Check_MK monitoring-aggregation connectors — for clients who already run Wazuh (SIEM/XDR: intrusion detection, log analysis, file-integrity monitoring), most valuable for Windows hosts where this app's own visibility is limited to patch counts. Deliberately scoped to ONLY the alert stream (Wazuh Indexer's wazuh-alerts-* index, rule.level >= 8 by default - Wazuh's own documented "requires investigation" threshold) - NOT vulnerability/SCA/inventory data from Wazuh's separate Manager API, since that overlaps with what this app already collects itself (vuln_scan.py/compliance.py/inventory.py) and would just create two disagreeing sources for the same question. New alerts feed the same alert_events -> Central -> Telegram pipeline as every other monitor here, correlated by IP against known Device/LinuxHost/WindowsHost records. Structurally different from PRTG/Check_MK internally: those track a persistent state (sensor up/down) and diff it between polls, but a Wazuh alert is a one-off event, so this uses a timestamp watermark instead (like tailing a log) - starts from the last hour only on first setup, never replays a deployment's full history.

## 2.24 — 2026-09-14
- Follow-up fixes from the codebase audit's "found but not auto-fixed" list. (1) Central per-user account login was structurally incomplete: "Wyloguj" only forgot the token in the browser tab (centralSession.clear()) without ever telling the server to revoke it, so a leaked/old token stayed valid until its 7-day TTL expired regardless of "logging out"; and there was no UI anywhere for the required me_totp_confirm step, meaning a per-user account's 2FA could be reset by an admin but could NEVER actually be turned on (totp_enabled only ever flips via that endpoint, self-service, by design). Logout now calls the server-side revoke first; a new "My account" bar (visible to every logged-in Central account, not just global admins) shows a code-entry box whenever a TOTP secret is set but not yet confirmed. ovh/api.php's /me now reports totp_enabled/totp_secret_set so the frontend knows when to show it. (2) Removed the old OVH-hosted AnyDesk admin panel's backend actions (status/sync-now/CSV-import/client-mapping add+delete/session-classify/summary) - confirmed dead: this whole feature was superseded by the local trace-file-based AnyDesk history added back in 1.59-1.62, and these 8 actions never got a UI after that switch. Kept the two actions the local page's one-time "Importuj z Centrali" bridge still reads (client-map list, sessions). (3) Wired up two backend endpoints that never got frontend UI: device neighbors (LLDP/CDP, /ip/neighbor) as a new tab on the device detail page, and router config backup history as a table under the Firmware tab (where backups are actually created).

## 2.23 — 2026-09-14
- Whole-project audit for dead code and missing bulk actions (backend, frontend, Central). Removed three confirmed-orphaned api.ts functions superseded by their streaming equivalents (vulnApi.run, linuxApi.discover, windowsApi.discover - the UI switched to live-progress EventSource endpoints a while ago and these plain-POST wrappers were never called from anywhere since). Added "check/scan/run all" bulk buttons alongside existing per-item actions, matching the pattern already used by the Linux/Windows host panels, for: Compliance (run all checks), Dell/iDRAC servers (both the local page and Central's cross-tenant panel), Central's supply-chain-scan panel, and Central's edge-device monitoring panel (check all enabled devices) - all read-only/safe actions, no bulk restart or other destructive action was added anywhere. Also fixed a real access-control gap in ovh/api.php: the "test alert channel" action (which actually sends a live Telegram message or hits a stored webhook URL) was missing the require_write() check present on every sibling channel-management action, so a read-only "viewer" Central account could trigger real outbound notifications.

## 2.22 — 2026-09-12
- Stop Central's "Skanuj"/scan-refresh buttons (Linux and Windows host panels) from triggering credential-guessing SSH/WinRM attempts against unknown devices on a client's network. Previously, both the "linux_scan" command AND the Windows panel's scan button (which had been mistakenly wired to the same "linux_scan" command, so it silently never did anything for Windows) called the full discover-new-hosts routine remotely - which tries the one shared credential against every device with an open port 22/5985/5986, including things like Mikrotik routers that aren't Linux/Windows servers at all, producing a wall of AuthenticationException log noise and needless failed-login attempts against devices Central has no business touching. New-host discovery is now agent-local only (the weekly automatic scan, or that agent's own "Skanuj sieć teraz" button) - Central's buttons now only refresh pending-update counts for hosts already marked as managed, via a proper new dedicated windows_scan command (mirroring linux_scan) instead of reusing the Linux one.

## 2.21 — 2026-09-12
- Fix stale Windows/Linux pending-update counts (e.g. Central showing "1" for a host that actually has 4-5 updates pending in its own Windows Update settings). Root cause: the actual apt/Windows-Update check that fills in `upgradable_count` only ever ran at vuln_scan's weekly network-discovery pass (or on a manual "Skanuj sieć teraz" click) - never on any tighter schedule, unlike disk/memory which already refresh every 30 minutes. So a managed host's badge could sit up to a week stale even though the check itself works correctly. Extracted the existing check-refresh logic out of discover_linux_hosts()/discover_windows_hosts() into standalone refresh_managed_hosts_updates() functions and wired them into resource_monitor.py's existing 30-minute Linux/Windows refresh loop (same cadence already used for disk/memory, no new SSH/WinRM connections needed beyond what that loop already opens). Also added a "last checked" timestamp to both the local UI and Central's Linux/Windows host tables (previously only "last upgrade" - i.e. last time an install actually ran - was shown, which stayed blank forever for anyone who hadn't yet installed anything and gave no way to tell how fresh the pending-count data itself was).
...
## 2.17 — 2026-09-09
- Pull PRTG's own log (sensor status changes, user actions, system messages) into this agent's local Activity Log, so PRTG's event history shows up alongside every other "what happened" entry instead of being a second place to check. New prtg_client.list_messages() (same table.json endpoint already used for sensors, just content=messages) and prtg_monitor.collect_activity() - deliberately separate from the existing up/down alerting, so routine PRTG log noise (e.g. "sensor paused by user") never reaches Telegram, only the local history. Check_MK's equivalent (its own audit log) is deferred for now: its REST API introduced an audit_log endpoint in 2.3.0b1, but the exact current path couldn't be verified against Checkmk's own source (no audit_log entry exists in the current endpoints directory) - shipping a second guessed endpoint after last week's two wrong Check_MK guesses wasn't worth repeating without being able to test it live.

## 2.16 — 2026-09-09
- Add agent-side self-checking for edge device (WAN) reachability, alongside OVH's own active probe - reported directly ("the address pings and responds, but I can manually confirm from my own network"). Root cause confirmed: OVH's shared hosting has no raw ICMP socket capability at all ("ping: socket: Address family not supported by protocol") and its own routing quirks to some destination networks (errno 101 Network unreachable), independent of whether the client's router is actually up. New services/edge_selfcheck.py lets each tenant's own agent - a normal OS process with a normal network stack, no such restriction - ping/TCP-check its own tenant's WAN addresses (a genuinely different, real network path for multi-site tenants like sanmed, whose branch routers connect over VPN tunnels) and report the result to OVH in its regular snapshot. OVH now tells each agent its full current (ip, check_port) list via a new edge_check_targets command (covers 'manual' entries too, which an agent otherwise has zero visibility into) - HMAC-signed through the same command channel already used for restart/upgrade/remediation commands. Purely additive: OVH's own active probe (edge_check_due) is unchanged and keeps running as an independent fallback for agents that haven't updated yet - both feed the exact same debounce/notification state machine (extracted into a new edge_apply_check_result()), so the fresher of the two (typically the agent, reporting every ~2 min vs OVH's own 15 min default) naturally wins.
...
## 2.13 — 2026-09-07
- Let a vulnerability finding's remediation status (accepted risk / in progress / resolved, with a note) be set directly from Central, not just on each tenant's own agent - reported directly ("some vulnerabilities are justified/acceptable or mitigated elsewhere, e.g. SSH needs to stay open for login"). The per-finding status/note editor already existed locally on the Podatnosci page; the gap was that Central's cross-tenant view couldn't see or change it at all. New request_vuln_remediation/pending_vuln_remediations command, mirroring the existing Linux/Windows restart command pattern exactly (queued marker file, delivered on the next heartbeat, HMAC-signed), lets Central set a finding's status for any tenant it has write access to. The write logic itself was extracted into a single reusable function (services/vuln_scan.py's set_remediation_status()) so both the local UI's endpoint and this new remote command go through the exact same code path.
